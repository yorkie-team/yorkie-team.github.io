...
In this guide, you will install Yorkie cluster on your local machine using Helm in Minikube.
Then, you will test Yorkie cluster with a simple example.
...
Now that you have Yorkie cluster installed and exposed, you can test Yorkie cluster with a simple example.
...
After dependencies are installed, change `VITE_APP_YORKIE_RPC_ADDR` in `.env.development` file.
