...
**Key Characteristics:**
- Server creates and deletes documents via [Admin API](/docs/web-api#documents-management)
...
> **Security Note:** When using Admin API for server-managed lifecycle, secure your API endpoints properly. See [Auth Webhook](/docs/security#auth-webhook) for authentication setup.
...
2. Server creates a new document through [Yorkie Admin API](/docs/web-api#post-yorkiev1adminservicecreatedocument)
...
1. Client or server removes document via [Yorkie Admin API](/docs/web-api#post-yorkiev1adminserviceremovedocument)
...
- [Web API](/docs/web-api) - Admin API for server-side management
