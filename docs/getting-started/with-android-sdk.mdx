...
val client = Client(
  host = "{{API_ADDR}}",
  options = Client.Options(apiKey = "xxxxxxxxxxxxxxxxxxxx"),
)
...
You can also subscribe to user presence using `document.events.filterIsInstance<Document.Event.PresenceChanged>()`. Use `document.presences` to get the list of users currently participating in the document.

```kotlin
// Declare your own CoroutineScope
scope.launch {
  document.events.filterIsInstance<Document.Event.PresenceChanged>().collect {
    val users = document.presences.value
    println("There are currently ${users.size} users online.")
  }
}
```
