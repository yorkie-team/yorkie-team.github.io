...
`@yorkie-js/react` is a React binding for the [Yorkie JS SDK](/docs/js-sdk) that provides a set of Providers and Hooks for integrating real-time collaboration into your React applications. It abstracts away the complexity of managing Yorkie Client and Document lifecycles, letting you focus on building your UI.

If you're new to Yorkie with React, start with the [Getting Started with React](/docs/getting-started/with-react) guide. For low-level JS SDK details (CRDT types, subscriptions, events), refer to the [JS SDK](/docs/js-sdk) guide.
...
| authTokenInjector | `(reason?: string) => Promise<string>` | No | undefined | Async function that returns an auth token for [Auth Webhook](/docs/security#auth-webhook) verification |
...
Use `authTokenInjector` to provide tokens for [Auth Webhook](/docs/security#auth-webhook) verification. If a `codes.Unauthenticated` error occurs, the injector is called again with the webhook's response reason, enabling automatic token refresh.
...
This means multiple clients can safely specify `initialRoot` without overwriting each other's data. For more details, see [JS SDK: Initializing root](/docs/js-sdk#initializing-root).
...
`ChannelProvider` attaches a [Channel](/docs/js-sdk#channel) to the client provided by `YorkieProvider`. The React hooks (`useChannel`, `useChannelSessionCount`) currently expose **session count tracking** only. For channel broadcast/subscribe functionality, use the [JS SDK Channel API](/docs/js-sdk#channel) directly.
...
| Prop | Type | Required | Default | Description |
|------|------|----------|---------|-------------|
| channelKey | string | Yes | - | Unique channel key (supports [hierarchical keys](/docs/js-sdk#hierarchical-channel-keys)) |
...
> Channel broadcast/subscribe hooks are planned for a future release. In the meantime, use the [JS SDK Channel API](/docs/js-sdk#channel) directly for messaging (and remember to unsubscribe/detach on unmount to avoid leaks).
...
For more on custom CRDT types (Text, Counter, Tree), see [JS SDK: Custom CRDT Types](/docs/js-sdk#custom-crdt-types).
...
For more on presence semantics (merging behavior, property replacement), see [JS SDK: Presence](/docs/js-sdk#presence).
