...
```bash
$ yorkie user login -u <username> --rpc-addr api.yorkie.dev:443
Enter Password:
```

The password is required in the next line. The password is not visible, which is normal.
...
> You can log in to the server using `yorkie login -u admin --insecure`. The password is required in the next line.
...
```bash
$ yorkie user change-password -u <username> --rpc-addr api.yorkie.dev:443
Enter Password:
Enter New Password:
```
...
```bash
$ yorkie user delete-account -u <username> --rpc-addr api.yorkie.dev:443
Enter Password:
WARNING:...
```
