...
As you can see above, if you do not enter a password with -p flag, you must enter it in the next line. The password is not visible, which is normal.
...
> You can log in to the server using `yorkie login -u admin -p admin --insecure`. Or you can try like this. `yorkie login -u admin --insecure`. 
In this case, you will need to enter a password in the next line.
...
If you want to change your password, you can use the command `passwd`. The both passwords are not visible, which is normal.
...
WARNING: ...
```
Likewise, it is normal that the password is not visible. If it fails, please double check if it is an existing account or a valid password.
