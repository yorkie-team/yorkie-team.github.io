...
Document represents a document accessible to the user. You can create, update and delete documents in the project.
...
Flags:
      --force   force remove a document even if attached in clients
...
If the document is attached to any client, you cannot remove it. You can use the `--force' flag to force a document to be removed.

```bash
$ yorkie document remove test-project test-document-1
Error: rpc error: ... remove document: document is attached

$ yorkie document remove test-project test-document-1 --force
```
