...
The password will not be visible, The password will not be visible, which is expected behavior.
...
If you want to change your password, you can use the command `passwd`. The both passwords will not be visible, which is expected.
...
WARNING: ...
```
As before, it is expected that the password will not be visible. If the operation fails, please double-check if it the account exists and whether the password is correct.
