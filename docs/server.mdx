...
Yorkie provides a sharded cluster mode in Kubernetes environments to support production environments.
...
- L7 Proxy Router: It is responsible for routing requests to servers based on the computed request’s hash key and consistent hashing algorithm.
- Service Registry: It is responsible for storing metadata and configuration settings for Yorkie servers.
- MongoDB: It stores the data of Yorkie. It can be configued to use sharding, just like Yorkie sharded cluster.
...
Also, you can additonally deploy Yorkie cluster addons on Kubernetes with Helm Charts on your own environment.

Yorkie cluster addons include:

- Metrics: Provides Yorkie metrics and logging via Grafana dashboard.
- Continous Delivery: Provides Yorkie Continous Delivery via ArgoCD.

Below is guide for deploying Yorkie cluster addons on Kubernetes with Helm Charts.

- [Yorkie Cluster Addons Installation](/docs/self-hosting-guide/cluster-addons): Guide to install Yorkie cluster addons on Kubernetes.
