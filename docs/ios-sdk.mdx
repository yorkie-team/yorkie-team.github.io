...
> For CLI users, you can create Clients without an API key. In this case, the Client will be created with a local rpcAddr
```swift
...
```
#### Syncing Documents if needed
In `SyncMode.manual`, the client does not automatically synchronize documents with the server. Therefore, you need to manually call the `sync(document:)` method to synchronize the document when necessary.
...
> However, it is allowed to use only `a-z`, `A-Z`, `0-9`, `-`, `.`, `_`, `~` and must be less than 120 characters (and more than 4 characters).
...
You can retrieve presence information for clients participating in the document.
