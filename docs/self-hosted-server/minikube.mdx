...
[Minikube](https://minikube.sigs.k8s.io/docs/) is a tool that makes it easy to run Kubernetes locally.
...
Before installing the Yorkie cluster with Helm chart, you also need to create a MongoDB namespace.

Create the `mongodb` namespace with the following command:

```bash
$ kubectl create namespace mongodb
```

This will create the `mongodb` namespace for MongoDB to be installed in during the Yorkie cluster Helm chart installation.
