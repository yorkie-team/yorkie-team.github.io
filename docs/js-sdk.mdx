...
console.log(root.obj);      // {"name":"josh","age":14}
console.log(root.obj.name); // "josh"
...
If you want to know more about changeInfo for document events, please refer to the [ChangeInfo](https://yorkie.dev/yorkie-js-sdk/api-reference/yorkie-js-sdk.remotechangeevent.html).

Additionally, you can subscribe to changes for a specific path, such as `$.todos`, where the `$` sign indicates the root of the document.
