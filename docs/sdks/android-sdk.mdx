...
> Since v0.7.15, `edit()` and `editByPath()` throw `YorkieException` with code `ErrInvalidArgument` when the given range or path resolves to an invalid split position (previously a raw `StringIndexOutOfBoundsException` could surface).
