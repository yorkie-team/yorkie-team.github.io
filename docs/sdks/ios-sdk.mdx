...
- `client.getActorID()` is the **stable actor**, supplied by the server on activation and derived from the project and the client key. It survives across sessions, and it is what the SDK stamps into the document's changes.
...
By default a Document lives only in memory: if the app is killed with changes that have not reached the server, those changes are gone. Set `ClientOptions.store` and the client persists every document attached to it, so edits made offline survive a restart and are pushed on the next successful sync.
...
`MemoryDocStore` is the built-in implementation, and it is deliberately **not** durable — it keeps documents in a process-local dictionary so the resume path is usable without the SDK committing to a storage location. To survive process restarts, supply your own `DocStore`.
