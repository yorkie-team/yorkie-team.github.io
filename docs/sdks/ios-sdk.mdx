...
The initializer also accepts a `DocumentOptions` value to seed document-scope settings:

```swift
let doc = Document(key: "doc-1", opts: DocumentOptions(disableGC: false, disablePresence: true))
```

- `disableGC`: Disables garbage collection for this document. See [Disabling GC](#disabling-gc).
