...
- **Not available during update**: calling `undo()` or `redo()` inside a `doc.update` callback throws a `YorkieError` with code `.errRefused` ("Undo is not allowed during an update").
